...
const fetchUsers = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.detail || `Error fetching users: ${response.status}`
      );
    }
    ...
    const getUsers = async () => {
      try {
        ...
        if (err.message && err.message.includes("404")) {
          setError(
            "The users API endpoint is not available. This feature may not be fully implemented yet."
          );
        } else {
      ...
      <Box sx={{ p: 3 }}>
                        ...
                        <TableRow key={user?.id || "unknown"}>
                          ...
                          <TableCell align="right">
                            <Button
                              ...
                              onClick={() =>
                                router.push(
                                  `/platform-admin/users/edit/${user?.id}`
                                )
                              }
                            >
                              Edit
        ...
        {!loading && !error && (
          <Alert severity="info" sx={{ mt: 3 }}>
            Note: The users API endpoint may not be fully implemented in the
            backend yet. This is a placeholder UI that will work once the API is
            available.
          </Alert>
        )}
      </Box>
