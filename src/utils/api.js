...
export const loginTenantAdmin = async (email, password, tenantName) => {
  try {
    ...
    console.log(
      `Attempting tenant admin login for ${email} in tenant ${tenantName}`
    );
...
export const requestTenantAdminPasswordReset = async (email, tenantName) => {
  try {
    ...
    console.log(
      `Requesting password reset for ${email} in tenant ${tenantName}`
    );
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.data && error.response.data.detail) {
        ...
        throw new Error(
          `You do not have permission to access the dashboard for "${tenantName}".`
        );
...
export { checkEmailExists as checkEmailAvailability, api };
