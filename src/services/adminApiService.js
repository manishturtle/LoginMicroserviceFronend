...
export const createCrmClient = async (clientData) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Error response from server:", errorData);
      throw new Error(
        errorData.detail || `Error creating CRM client: ${response.status}`
      );
    }
...
export const getCrmClientDetails = async (clientId) => {
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/platform-admin/api/crmclients/${clientId}/`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.detail ||
          `Error fetching CRM client details: ${response.status}`
      );
    }
...
export const updateCrmClient = async (clientId, clientData) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || `Error updating CRM client: ${response.status}`
      );
    }
...
export const deleteCrmClient = async (clientId) => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
        errorData.detail || `Error deleting CRM client: ${response.status}`
      );
    }
  } catch (error) {
