...
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL || "http://localhost:8012/api";
 ...
 * @returns {Promise<Object>} The login response data
 */
export const verifyTenantAdminTwoFactorLogin = async (
  userId,
  code,
  tenantName
) => {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.detail ||
        errorData.message ||
        "Failed to verify tenant admin 2FA code"
    );
  }
...
export const generateNewRecoveryCodes = async (password) => {
  const response = await fetch(
    `${API_BASE_URL}/auth/2fa/recovery-codes/generate/`,
    {
      method: "POST",
      headers: getAuthHeaders(),
      credentials: "include", // Include cookies for authentication
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(
      errorData.message || "Failed to generate new recovery codes"
    );
  }
...
export const verifyAndCompleteSignup = async ({
  ...
  try {
    ...
    console.log("Verifying and completing signup with:", {
      email,
      userId,
      verificationCode,
      hasBackupSecret: !!backupSecret,
    });

    const response = await fetch(
      `${API_BASE_URL}/auth/signup/verify-and-complete/`,
      {
        method: "POST",
        headers,
        body: JSON.stringify(data),
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || "Failed to verify and complete signup"
      );
    }
