...
export const fetchTenantUsers = async (options = {}) => {
  try {
    ...
    const response = await api.get(
      `/tenant-admin/users/?${params.toString()}`,
      {
        headers: {
          "X-Tenant-Name": "default",
          "X-Tenant-Admin": "true",
...
export const createTenantUser = async (userData, tenant) => {
  try {
    ...
    const response = await fetch(
      `http://localhost:8012/api/${tenant}/tenant-admin/users/`,
      {
        method: "POST",
        headers: {
...
export const fetchInvitations = async (params = {}) => {
  try {
    const response = await api.get("/tenant-admin/invitations/", {
      params,
      headers: {
        "X-Tenant-Name": "default",
        "X-Tenant-Admin": "true",
...
export const fetchTenantReports = async (params = {}) => {
  try {
    const response = await api.get("/tenant-admin/reports/", {
      params,
      headers: {
        "X-Tenant-Name": "default",
        "X-Tenant-Admin": "true",
...
export const fetchTenantRoles = async (tenant) => {
  ...
  try {
    ...
    try {
      const response = await fetch(
        `http://localhost:8012/api/${tenant}/tenant-admin/roles/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      const debugResponse = await fetch(
        `http://localhost:8012/api/${tenant}/tenant-admin/debug-roles/`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      console.log(
        `Debug roles endpoint response status: ${debugResponse.status}`
      );

      if (debugResponse.ok) {
        const debugData = await debugResponse.json();
        console.log(
          "Roles fetched successfully from debug endpoint:",
          debugData
        );
    ...
    const response = await fetch(
      `http://localhost:8012/api/${tenant}/tenant-admin/tenant-admin-roles/`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    console.log(
      `Tenant admin roles endpoint response status: ${response.status}`
    );
    ...
    if (!response.ok) {
      ...
      try {
        ...
        try {
          const errorText = await response.text();
          console.error(
            "Tenant admin roles API error response (text):",
            errorText
          );
...
export const loginAsTenantAdmin = async (email, password, tenant) => {
  try {
    console.log(
      `Attempting to login as tenant admin: ${email} in tenant: ${tenant}`
    );

    // Make the login request
    const response = await fetch(
      `http://localhost:8012/api/${tenant}/tenant-admin/auth/login/`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
 ...
 * @returns {Promise<Object>} Created user data
 */
export const createTenantUserWithLogin = async (
  userData,
  tenant,
  adminEmail,
  adminPassword
  ...
  try {
    ...
    const loginResponse = await loginAsTenantAdmin(
      adminEmail,
      adminPassword,
      tenant
    );
